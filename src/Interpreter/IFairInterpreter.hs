...
iFairInterpreter :: CExp -> BType -> IO ()
iFairInterpreter cExp expectedBType =
  do
    ...
    iFairInterpreterHelper exp s l0 1 1 expectedBType

iFairInterpreterHelper :: CExp -> Store -> CExp -> Integer -> Integer -> BType -> IO ()
iFairInterpreterHelper cExp s location mode nowNum expectedBType = do
  ...
  iFairInterpreterHelper cExp' s' l' mode' (nowNum + 1) expectedBType
